...
xMin = min(av - d - stdv,av-4*stdv);    % a sensible range of values
xMax = max(av + d + stdv,av+4*stdv);    % to plot the graph on
...
tX = xx(1);
tY = pdfMax;
text(tX,tY,['VaR = ', num2str(VaR)],'fontSize',12, 'fontWeight','bold');
tY = 1.075*pdfMax;
