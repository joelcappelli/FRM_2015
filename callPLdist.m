%
% this script shows the distribution of PL for a call option assuming the
...
callPrice = bsCallPrice(spot, strike, rate, div, vol, expiry);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

mu     = 0.10;    % underlying asset's ANNUAL growth rate

sims   = 1000000;  % number of price changes to simulate
...
expiry1 = expiry-dt;
if (expiry1 > 0) 
    dV = bsCallPrice(ST, strike, rate, div, vol, expiry1) - callPrice;
else
    dV = bsCallPayoff(ST, strike) - callPrice;
end
...
legend('Option','Call','location','NW');
